...
This example configures MASTER_SPI as master and SLAVE_SPI as slave, remember to set SEL as output mode
for slave select. Both SPIs are configured with 8 bits mode, FIFO mode with Tx & Rx FIFO depths
4, (PCLK_SPI/4) transmission speed.

Transmission begins when MASTER_SPI starts to send data from SPI0_Buffer_Tx and receives 4 data to
SPI0_Buffer_Rx a time in one ISR, This is because Rx FIFO depth is configured as 4. The same
behavior slave SLAVE_SPI does, Transmission by polling, Receiving by interrupt, This example jumps
...
- Remember to connect MASTER_SPI to SLAVE_SPI respectively. Refer "ht32_board_config.h" for pin assignment.
