...
/* Settings ------------------------------------------------------------------------------------------------*/
/* Private types -------------------------------------------------------------------------------------------*/
typedef void (*pTimeFunPtr)();
typedef struct TIMEEVENT
{
	u8 active;
	u32 inittick;  // restore the tick
	u32 curtick;	 // systick counter
	pTimeFunPtr timefun;
} TimerEvent;
...
int main(void)
{
  ...
  SysTickEvent[0].active = TRUE;
	SysTickEvent[0].inittick = 2;  
	SysTickEvent[0].curtick = 2;
	SysTickEvent[0].timefun = (pTimeFunPtr)LED1_Toggle;  
...
void SysTick_COnfiguration(void)
{
  SYSTICK_ClockSourceConfig(SYSTICK_SRC_STCLK); // 6 MHz on chip
  SYSTICK_SetReloadValue(6000000);              
...
void TimerEvent_Handler(void)
{
	...
	for (i = 0; i < 10; i++)
	{
		if (SysTickEvent[i].active)
		{
			SysTickEvent[i].curtick--;
			if (SysTickEvent[i].curtick == 0)
			{
				(*SysTickEvent[i].timefun)();
				SysTickEvent[i].curtick = SysTickEvent[i].inittick;
			}
		}
	}
}
